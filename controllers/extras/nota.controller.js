...
const getNotaAll = async (req, res) => {
  try {
    const notas = await NotaModel.getAllRaw()
...
const updateNota = async (req, res) => {
  try {
    ...
    if (valor !== undefined) {
      notas[indice].valor = Number(valor)
    }
...
const deleteNota = async (req, res) => {
  try {
    ...
    let notas = await NotaModel.getAllRaw()
