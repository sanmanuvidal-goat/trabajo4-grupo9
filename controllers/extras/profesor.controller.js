...
const getProfesorAll = async (req, res) => {
  try {
    const profesores = await ProfesorModel.getAllRaw()
...
const createProfesor = async (req, res) => {
  try {
    ...
    if (profesores.some((p) => p.id === Number(id))) {
      return res.status(409).json({ error: `El profesor con ID ${id} ya existe` })
    }
    ...
    await ProfesorModel.saveAll(profesores)
    ...
    console.error(error)
    return res.status(500).json({ error: 'Error interno al registrar al profesor' })
  }
}
...
const updateProfesor = async (req, res) => {
  try {
    ...
    console.error(error)
    return res.status(500).json({ error: 'Error interno al modificar al profesor' })
  }
}
...
const deleteProfesor = async (req, res) => {
  try {
    ...
    let profesores = await ProfesorModel.getAllRaw()
    ...
    return res.status(200).json({
      msg: `Profesor con ID ${id} eliminado`
    })

  } catch (error) {
    console.error(error)
    return res.status(500).json({ error: 'Error interno al eliminar al profesor' })
  }
}
