...
const getMateriaAll = async (req, res) => {
  try {
    const materias = MateriaModel.getAllRaw();
...
const createMateria = async (req, res) => {
  try {
    ...
    if (!id || !nombre || !cuatrimestre) {
      return res.status(400).json({ error: 'Todos los campos (id, nombre, cuatrimestre) son obligatorios' });
    }

    const materias = MateriaModel.getAllRaw();
    if (materias.some(m => m.id === Number(id))) {
      return res.status(409).json({ error: `La materia con ID ${id} ya existe` });
    }

    const nuevaMateria = { id: Number(id), nombre, cuatrimestre, isActive: true };
    materias.push(nuevaMateria);
    MateriaModel.saveAll(materias);

    return res.status(201).json({ msg: 'Materia creada con éxito', materia: nuevaMateria });
  } catch (error) {
    console.error(error);
...
const updateMateria = async (req, res) => {
  try {
    ...
    MateriaModel.saveAll(materias);
    return res.status(200).json({ msg: 'Materia modificada con éxito', materia: materias[indice] });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ error: 'Error interno al modificar la materia' });
  }
};
...
const deleteMateria = async (req, res) => {
  try {
    const { id } = req.params;
    let materias = MateriaModel.getAllRaw();
    ...
    MateriaModel.saveAll(materias);
    return res.status(200).json({ msg: `Materia con ID ${id} eliminada` });
  } catch (error) {
    console.error(error);
    return res.status(500).json({ error: 'Error interno al eliminar la materia' });
  }
};

module.exports = { getMateriaAll, getMateriaById, createMateria, updateMateria, deleteMateria };